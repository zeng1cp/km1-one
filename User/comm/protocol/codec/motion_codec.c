...
bool proto_decode_motion_cycle_create(const uint8_t*                   payload,
                                      uint16_t                         len,
                                             ...
                                             uint16_t                              buf_size)
{
    ...
    for (uint8_t i = 0; i < resp->cycle_count; ++i) {
        const proto_motion_cycle_status_t* st = &resp->cycles[i];
...
uint16_t proto_encode_motion_cycle_status_update_resp(
    const proto_motion_cycle_status_update_resp_t* resp, uint8_t* buf, uint16_t buf_size)
{
